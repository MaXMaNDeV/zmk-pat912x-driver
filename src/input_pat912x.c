...
static void pat912x_motion_work_handler(struct k_work *work)
{
	...
	x = pat912x_sign_extend(x, PAT912X_DATA_SIZE_BITS - 1);
	y = pat912x_sign_extend(y, PAT912X_DATA_SIZE_BITS - 1);
...
static int pat912x_init(const struct device *dev)
{
	...
	/* Zephyr 3.5 compatibility: PM runtime may not be available */
